...
class PaginationUtil {
  ...
  public getPaginationLinks(location: string, paginationParams: PaginationParamsInterface, pageMax: number): any {
    return {
      self: PaginationUtil.buildLink(location, paginationParams),
      first: PaginationUtil.buildLink(location, { page: 1, limit: paginationParams.limit }),
      last: PaginationUtil.buildLink(location, { page: pageMax, limit: paginationParams.limit }),
      next: PaginationUtil.buildLink(location,
        {
          page: paginationParams.page === pageMax ? pageMax : paginationParams.page + 1,
          limit: paginationParams.limit,
        }),
      prev: PaginationUtil.buildLink(location,
        {
